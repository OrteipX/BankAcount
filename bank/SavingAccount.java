 ...
 * Purpose: a saving account with provides more interest
...
public class SavingAccount extends BankAccount
{
    ...
    public void calcInterest()
    {
        ...
        if (balance >= 25)
        {
            if (balance >= 2000)
            {
                // check if balance has been greater than $2,000.00 the whole month
                for (Transaction t : record)
                {
                    if (t.getBalance() < 2000)
                    {
                        applyBonus = false;
                        break;
                    }
                ...
                // assign or not the bonus regarding to monthly balance
                INT_RATE = applyBonus ? INT_RATE += 0.0075 : INT_RATE;
            }
